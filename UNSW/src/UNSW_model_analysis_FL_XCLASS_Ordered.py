...
classes_str = str(pd.read_csv(sys.argv[2])['Class List'].to_list()[int(sys.argv[1])])[2:-2]
...
def analyze_model_n_packets(npkts, outfile, feats_to_use, time):    
    ...
    ###########
    # print(flow_pkt_counts['label'].value_counts())
...
for nd in range(2,5):
    ...
    f_name = "/home/nds-admin/UNSW_PCAPS/hyb_code/with_FL_Metric/cluster_model_analysis_results/Proportional_fairness/unsw_models_"+str(nd)+"pkts_PF_WB_24CL_Cluster"+str(int(sys.argv[1])-1)+"_MACRO_Ordered_withOptimizer_NOdelta_13June_53146720.csv"
