...
feats_important = ['ip.len', 'tcp.window_size_value', 'udp.length', 'dstport', 'srcport', 'ip.ttl', 'tcp.hdr_len',\
                   'Packet Length Total', 'Packet Length Mean', 'Max Packet Length', 'tcp.flags.rst', 'Flow IAT Mean',\
...
def get_feature_importance(depth, n_tree, max_leaf, X_train, y_train, weight_of_samples):
    ...
    rf_opt = RandomForestClassifier(max_depth = depth, n_estimators = n_tree, max_leaf_nodes=max_leaf, random_state=42, bootstrap=False,n_jobs=12)
...
def get_scores(classes, depth, n_tree, feats, max_leaf, X_train, y_train, X_test, y_test, unique_labels,array_of_indices,weight_of_samples):
    model = RandomForestClassifier(max_depth=depth, n_estimators = n_tree, max_leaf_nodes=max_leaf, n_jobs=12,
...
#     return num_samples, macro_f1, weighted_f1, micro_f1
...
def compute_flow_pkt_scores(y_pred, y_test, sample_nature,unique_labels,array_of_indices):
    ...
    # labels=array_of_indices, target_names=unique_labels,
    c_report_pkt = classification_report(pkt_df_y_true, pkt_df_y_pred, labels=unique_labels, target_names=array_of_indices, output_dict=True)
    pkt_macro_f1 = c_report_pkt['macro avg']['f1-score']
    pkt_weighted_f1 = c_report_pkt['weighted avg']['f1-score']
    ...
    c_report_flw = classification_report(flw_df_y_true, flw_df_y_pred, labels=unique_labels, target_names=array_of_indices, output_dict=True)
    flw_macro_f1 = c_report_flw['macro avg']['f1-score']
    flw_weighted_f1 = c_report_flw['weighted avg']['f1-score']
...
def analyze_models(classes, model_type, depths, n_trees, X_train, y_train, X_test, y_test, samples_nature, y_multiply, test_flow_pkt_cnt, test_flow_IDs, max_leaf,  test_labels, test_indices, max_feats, filename, weight_of_samples):
    ...
    with open(filename, "w") as res_file:
        print('depth;tree;no_feats;N_Leaves;Macro_f1_FL;Weighted_f1_FL;Micro_f1_FL;feats;n_of_leaves;pkt_macro_f1;pkt_weighted_f1;flw_macro_f1;flw_weighted_f1;F1_macro;F1_weighted;num_samples;Macro_F1_PL;Weighted_F1_PL;Micro_F1_PL;cl_report_FL;cl_report_PL', file=res_file)
        if model_type == 'RF':
                ...
                for depth in depths:
                    for leaf in max_leaf:
                        ...
                        for feats in m_feats:
                            ...
                            model, c_report, macro_f1, weight_f1, y_pred, n_of_leaves = get_scores(classes, depth, n_tree, feats, leaf, X_train, y_train, X_test, y_test,  test_indices, test_labels, weight_of_samples)
                            #
                            ...
                            # num_samples, macro_f1_FL, weighted_f1_FL, micro_f1_FL, test_df = calculate_FL_metric_score(y_test, y_pred, samples_nature, y_multiply, test_flow_pkt_cnt, test_flow_IDs, test_indices, test_labels)
                            #
                            # return test_df, macro_f1_FL, weighted_f1_FL, micro_f1_FL, macro_f1_PL, weighted_f1_PL
                            print(str(depth)+';'+str(n_tree)+';'+str(len(feats))+';'+str(leaf)+";"+str(macro_f1_FL)+";"+str(weighted_f1_FL)+";"+str(micro_f1_FL)+";"+str(list(feats))+';'+str(n_of_leaves)+';'+str(pkt_weighted_f1)+';'+str(flw_macro_f1)+';'+str(flw_weighted_f1)+';'+str(macro_f1)+';'+str(weight_f1)+';'+str(num_samples)+';'+str(macro_f1_PL)+';'+str(weighted_f1_PL)+';'+str(micro_f1_PL)+';'+str(cl_report_FL)+';'+str(cl_report_PL), file=res_file)
...
for nd in range(5,6):
    ...
    f_name = "/home/nds-admin/UNSW_PCAPS/hyb_code/with_FL_Metric/model_analysis_results/unsw_models_"+str(nd)+"pkts_FEATSALL_FL.csv"
    analyze_model_n_packets(nd, f_name, feats_all, "normal")
