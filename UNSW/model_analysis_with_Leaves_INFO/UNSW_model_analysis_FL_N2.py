...
def calculate_FL_metric_score(y_true, y_pred, sample_nature, multiply, test_flow_pkt_cnt, test_flow_IDs, unique_labels, array_of_indices):
    ...
    return num_samples, macro_f1, weighted_f1, micro_f1, test_df
...
def analyze_models(classes, model_type, depths, n_trees, X_train, y_train, X_test, y_test, samples_nature, y_multiply, test_flow_pkt_cnt, test_flow_IDs, max_leaf,  test_labels, test_indices, max_feats, filename, weight_of_samples):
    ...
    with open(filename, "w") as res_file:
        print('depth;tree;no_feats;N_Leaves;Macro_f1_FL;Weighted_f1_FL;Micro_f1_FL;feats;n_of_leaves;pkt_macro_f1;pkt_weighted_f1;flw_macro_f1;flw_weighted_f1;F1_macro;F1_weighted;num_samples;Macro_F1_PL;Weighted_F1_PL', file=res_file)
        if model_type == 'RF':
                ...
                for depth in depths:
                    for leaf in max_leaf:
                        ...
                        for feats in m_feats:
                            ...
                            num_samples, macro_f1_PL, weighted_f1_PL = expand_rows_and_get_scores(y_test, y_pred, samples_nature, y_multiply, test_indices, test_labels)
                            ...
                            print(str(depth)+';'+str(n_tree)+';'+str(len(feats))+';'+str(leaf)+";"+str(macro_f1_FL)+";"+str(weighted_f1_FL)+";"+str(micro_f1_FL)+";"+str(list(feats))+';'+str(n_of_leaves)+';'+str(pkt_macro_f1)+';'+str(pkt_weighted_f1)+';'+str(flw_macro_f1)+';'+str(flw_weighted_f1)+';'+str(macro_f1)+';'+str(weight_f1)+';'+str(num_samples)+';'+str(macro_f1_PL)+';'+str(weighted_f1_PL), file=res_file)
...
analyze_model_n_packets(2, f_name, feats_all, "normal", depths, trees)
